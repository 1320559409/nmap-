#!/bin/bash
# 通用TCP服务探测脚本（支持IPv4和IPv6）
# 用法: ./svc_t.sh <ipv4|ipv6>
...
if [ "$IP_VERSION" = "ipv4" ]; then
    GNMAP="$OUT_DIR/ipv4_t.gnmap"
    OUTFILE="$OUT_DIR/svc_ipv4_t"
    COMPLETED_FILE="$OUT_DIR/.completed_t_ipv4"
else
    GNMAP="$OUT_DIR/ipv6_t.gnmap"
    OUTFILE="$OUT_DIR/svc_ipv6_t"
    COMPLETED_FILE="$OUT_DIR/.completed_t_ipv6"
fi

TMPDIR_WORK="$OUT_DIR/tmp_t_${IP_VERSION}_$$"

# 检查gnmap文件（端口扫描结果）
if [ ! -f "$GNMAP" ]; then
    echo "警告: $GNMAP 不存在，请先运行端口扫描 (scan_t.sh)" >&2
    touch "$OUTFILE"
    exit 0
fi
...
echo "开始 ${IP_VERSION^^} TCP 服务探测，共 $TOTAL_IPS 个 IP" >&2
